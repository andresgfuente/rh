        ...
        if not job_description.strip():
            ...
            with st.spinner('Espera un rato...'):
                ...
                for n in range(len(final_docs_list)):
                    ...
                    try:
                        if len(final_docs_list[n].page_content)>15:
                                ...
                                fortalezas = dictionario_texto.get("Fortalezas", "No se menciona")
                                debilidades = dictionario_texto.get("Debilidades", "No se menciona")
                                categoria = dictionario_texto.get("Categoria", "No se menciona")
                            ...
                            st.error(f'Error al procesar el Cv:  {pdf[n].name}, por favor revise que el contenido sea correcto.')
                            progress = (n + 1) / len(final_docs_list)
                            progress_bar.progress(progress)
                            percent_text.text(f"Progreso: {int(progress * 100)}%")
                            pass
